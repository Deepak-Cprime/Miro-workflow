...
export class OpenAIAnalyzer {
    ...
    buildAnalysisPrompt(workflowData) {
        const nodesDescription = workflowData.nodes.map(node => `- Node ${node.id}: "${node.title}" (Type: ${node.type})`).join('\n');
        return `
...
`;
    ...
    async generateWorkflowReport(workflowData, openaiInsights) {
        const reportPrompt = `
...
${insights.epics.length > 0
            ? insights.epics.map(epic => `#### ${epic.title} (${epic.epicId})\n${epic.description}`).join('\n\n')
            : 'No epics identified'}

### Features and User Stories
${insights.features.length > 0
            ? insights.features.map(feature => `#### ${feature.title} (${feature.featureId})\n${feature.description}\n\n**User Stories:**\n${feature.userStories.map(story => `- **${story.title}** (${story.storyId})\n  ${story.asA}, ${story.iWant} ${story.soThat}${story.tasks && story.tasks.length > 0 ? `\n  **Tasks:**\n${story.tasks.map(task => `    - ${task.title}`).join('\n')}` : ''}`).join('\n\n')}`).join('\n\n')
